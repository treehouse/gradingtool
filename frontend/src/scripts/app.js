...
const lameAss = document.getElementById("im-lame");

newApp.addEventListener("click", () => {
  window.open("https://grading-tool.netlify.app/");
});

lameAss.addEventListener("click", () => {
  document.querySelector(".new-overlay").style.display = "none";
});
...
const projectFilesContainer = document.querySelector(
  ".project-files-container"
);
projectFilesContainer.addEventListener("click", (e) => {
  if (
    e.target.classList.contains("mockup-type") ||
    e.target.parentNode.classList.contains("mockup-type")
  ) {
    toggleCarousel();
  }
});
  ...
  tds.forEach((td) => {
    if (e.target == td) {
      ...
      if (document.querySelector(".td-project-list .loader")) {
        document.querySelector(".td-project-list .loader").style.display =
          "block";
      }
      loadProjectList(id);
      hideViews();
  ...
  function populate(data, id) {
    ...
    if (!data.projects?.length) {
      ...
      requirementList.innerHTML =
        '<p class="no-data-message">There is no data for this Techdegree.</p>';
    } else {
      // toggleDropdown();
      requirementList.innerHTML = "";
...
function handleGrading(e) {
  ...
  let currentTextarea =
    e.target.parentNode.parentNode.querySelector("textarea");
  btns.forEach((btn) => {
...
function loadProjectRequirements(data) {
  ...
  if (!data.gradingSections?.length) {
    requirementList.innerHTML =
      '<p class="no-data-message">Oops! There is no data for this project.</p>';
  } else {
    data.gradingSections.forEach((section, sectionIndex) => {
      ...
      section.requirements.forEach((req) => {
        ...
        textarea.setAttribute(
          "placeholder",
          "Optional - write a helpful message about your grade selection."
        );
...
const utilitySectionTitles = document.querySelectorAll(
  "[data-util-section-title]"
);
...
const reviewHeader_toggle_correct = document.querySelector(
  "[data-show-correct]"
);
const reviewHeader_toggle_question = document.querySelector(
  "[data-show-question]"
);
...
function handleReviewToggles(e) {
  ...
  const questionedItemsParent = document.querySelector(
    ".questioned-items-container"
  );
  const incorrectItemsParent = document.querySelector(
    ".incorrect-items-container"
  );
...
function copySlackMessage() {
  ...
  if (toggle_question.classList.contains("active")) {
    gradedData.questionableItems.forEach((item) => {
      ...
      secretTextarea.value += `:questioned: ${isExceeds ? ":exceeds:" : ""} ${
  ...
  if (toggle_wrong.classList.contains("active")) {
    gradedData.incorrectItems.forEach((item) => {
      const requirement = item.req.querySelector(".sub-requirements-title");
      const isExceeds = item.req.classList.contains("exceeds-item");
      secretTextarea.value += `:needs-work: ${isExceeds ? ":exceeds:" : ""} ${
        requirement.textContent
      }\n> ${item.text}\n`;
    });
  }
...
const noteForm_contents = document.querySelector(
  "form.new-note-form #noteContents"
);
